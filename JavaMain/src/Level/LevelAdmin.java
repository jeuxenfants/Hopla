 ...
 */
public class LevelAdmin extends JFrame {
    ...
    private final String name = "Level Administration";//Nom de la fenetre en mode admin
    String image = "backlevel.jpg"; // background
    java.applet.AudioClip son=null;

    public LevelAdmin() {
        java.net.URL url = LevelAdmin.class.getResource("magic.wav");
        ...
        boutons.setPreferredSize(new Dimension(250,600));
        ...
        L2.addActionListener(new MonListener());
        L2.setFont(new Font("sansserif", Font.BOLD, 35)); //Changement de la police et de la taille
        retour = new JButton("retour");//Création d'un bouton de retour qui permet de revenir au menu PrincipalAdmin
        retour.addActionListener(new MonListener());//Ajout d'un ecouteur le boutton de niveau 2
        retour.setFont(new Font("sansserif", Font.BOLD, 35)); //Changement de la police et de la taille
        
        boutons.add(L1, BorderLayout.LINE_START);//Ajout des bouttons dans le Panel de boutton avec organisation de leurs emplacements
        boutons.add(Box.createVerticalStrut(50));
        boutons.add(L2, BorderLayout.LINE_END);
        boutons.add(Box.createVerticalStrut(50));
        boutons.add(retour, BorderLayout.PAGE_END);

        affichagePanel.add(affichage);//ajout d'un panel dans le label affichage

        content.add(boutons, BorderLayout.EAST);
    ...
    private class MonListener implements ActionListener {//méthode de gestion des actions sur les boutons
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if ("LEVEL 1".equals(s)) {//Comparaison du texte suivant avec le texte du bouton pour déclencher une action si c'est le même texte
                 son.play();
                JOptionPane.showMessageDialog(null, "Vous avez choix le " + s);//Pop up qui définit le choix utilisateur 

                setVisible(false);//Rendre la fenetre active invisible avant la création d'une autre fenetre 
            ...
            if ("LEVEL 2".equals(s)) {//Comparaison du texte suivant avec le texte du bouton pour déclencher une action si c'est le même texte
                JOptionPane.showMessageDialog(null, "Vous avez choix le " + s);//Pop up qui définit le choix utilisateur 
                setVisible(false);//Rendre la fenetre active invisible avant la création d'une autre fenetre
                MenuPrincipalAdmin menu = new MenuPrincipalAdmin();//création d'un menu Principal Admin
            }
